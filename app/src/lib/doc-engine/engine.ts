...
export const CHECK_MARK = { glyph: 'X', size: 7, capHeight: 0.718 } as const;
 ...
 */
export function validateEncoding(text: string): string[] {
  const bad: string[] = [];
 ...
 */
export async function generateDetailed(input: GenerateInput): Promise<GenerateResult> {
  ...
  if (encodingIssues.length) {
    const err = new Error(
      'Cannot generate: non-encodable characters in ' +
  ...
  const pageOf = (n: number, key: string) => {
    const p = pages[n - 1];
    if (!p) throw new Error(`Template "${template.key}": "${key}" targets page ${n}, but the PDF has ${pages.length} page(s)`);
    return p;
  };
